...
export const inMemoryConsultations: any[] = [];
...
export const getConsultations = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (!consultations || consultations.length === 0) {
      consultations = inMemoryConsultations.filter(
        (c) =>
          !req.user ||
          req.user.role === 'ADMIN' ||
...
export const respondConsultation = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    if (!req.user) return;
...
export const getExpertsList = async (req: Request, res: Response): Promise<void> => {
  try {
    let experts: any[] = [];
    if (isMongoConnected()) {
      try {
