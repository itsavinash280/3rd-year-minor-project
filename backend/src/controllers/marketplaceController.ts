...
export const inMemoryListings: any[] = [];
export const inMemoryCarts: Map<string, any> = new Map();
export const inMemoryWishlists: Map<string, any[]> = new Map();
  ...
  try {
    ...
    if (isMongoConnected()) {
      try {
        ...
        if (dbListing) listing = dbListing as any;
    ...
    const existingIdx = cart.items.findIndex((item: any) => item.listingId === listingId || item.listingId?._id === listingId);
    if (existingIdx > -1) {
      cart.items[existingIdx].quantity += Number(quantity);
    } else {
      cart.items.push({
...
export const removeFromCart = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (cart) {
      cart.items = cart.items.filter((item: any) => (item.listingId?._id || item.listingId) !== listingId);
      inMemoryCarts.set(userId, cart);
    }
...
export const toggleWishlist = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const idx = items.findIndex((i: any) => (i._id || i) === listingId);
    if (idx > -1) {
      items.splice(idx, 1);
      isAdded = false;
    } else {
