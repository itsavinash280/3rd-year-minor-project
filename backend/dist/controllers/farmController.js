import { FarmerProfile } from '../models/FarmerProfile.js';
import { isMongoConnected } from '../config/db.js';
export const inMemoryFarmProfiles = new Map();
export const getFarmProfile = async (req, res) => {
    try {
        if (!req.user)
            return;
        const userId = req.user._id ? req.user._id.toString() : req.user.id;
        let profile = null;
        if (isMongoConnected()) {
            try {
                profile = await FarmerProfile.findOne({ userId: req.user._id });
            }
            catch (dbErr) {
                console.warn('[MongoDB Get Farm Profile Fallback]:', dbErr);
            }
        }
        if (!profile) {
            profile = inMemoryFarmProfiles.get(userId) || {
                userId,
                farmName: `${req.user.name || 'Farmer'}'s Modern Farm`,
                farmSize: 5,
                sizeUnit: 'ACRES',
                soilType: 'ALLUVIAL',
                soilPh: 6.8,
                nitrogen: 140,
                phosphorus: 45,
                potassium: 40,
                irrigationMethod: 'CANAL',
                state: 'Uttar Pradesh',
                district: 'Lucknow',
                village: 'Malihabad',
                pincode: '226101',
                cropsGrown: ['Wheat', 'Paddy', 'Mustard'],
                farmingExperienceYears: 8,
                aadhaarMasked: 'XXXX-XXXX-9876',
                farmImages: ['https://images.unsplash.com/photo-1500382017468-9049fed747ef?auto=format&fit=crop&w=800&q=80'],
            };
        }
        res.status(200).json({ success: true, profile });
    }
    catch (error) {
        res.status(200).json({
            success: true,
            profile: {
                farmName: 'Organic Agri Farm',
                farmSize: 5,
                sizeUnit: 'ACRES',
                soilType: 'ALLUVIAL',
                cropsGrown: ['Wheat', 'Mustard'],
            },
        });
    }
};
export const createOrUpdateFarmProfile = async (req, res) => {
    try {
        if (!req.user)
            return;
        const userId = req.user._id ? req.user._id.toString() : req.user.id;
        const { farmName, farmSize, sizeUnit, soilType, soilPh, nitrogen, phosphorus, potassium, irrigationMethod, state, district, village, pincode, cropsGrown, farmingExperienceYears, aadhaarMasked, farmImages, } = req.body;
        let profile = null;
        if (isMongoConnected()) {
            try {
                profile = await FarmerProfile.findOne({ userId: req.user._id });
                if (profile) {
                    profile.farmName = farmName || profile.farmName;
                    profile.farmSize = farmSize || profile.farmSize;
                    profile.sizeUnit = sizeUnit || profile.sizeUnit;
                    profile.soilType = soilType || profile.soilType;
                    profile.soilPh = soilPh || profile.soilPh;
                    profile.nitrogen = nitrogen || profile.nitrogen;
                    profile.phosphorus = phosphorus || profile.phosphorus;
                    profile.potassium = potassium || profile.potassium;
                    profile.irrigationMethod = irrigationMethod || profile.irrigationMethod;
                    profile.state = state || profile.state;
                    profile.district = district || profile.district;
                    profile.village = village || profile.village;
                    profile.pincode = pincode || profile.pincode;
                    profile.cropsGrown = cropsGrown || profile.cropsGrown;
                    profile.farmingExperienceYears = farmingExperienceYears || profile.farmingExperienceYears;
                    if (aadhaarMasked)
                        profile.aadhaarMasked = aadhaarMasked;
                    if (farmImages)
                        profile.farmImages = farmImages;
                    await profile.save();
                }
                else {
                    profile = await FarmerProfile.create({
                        userId: req.user._id,
                        farmName: farmName || `${req.user.name}'s Organic Farm`,
                        farmSize: farmSize || 5,
                        sizeUnit: sizeUnit || 'ACRES',
                        soilType: soilType || 'ALLUVIAL',
                        soilPh: soilPh || 6.8,
                        nitrogen: nitrogen || 140,
                        phosphorus: phosphorus || 45,
                        potassium: potassium || 40,
                        irrigationMethod: irrigationMethod || 'CANAL',
                        state: state || 'Uttar Pradesh',
                        district: district || 'Lucknow',
                        village: village || 'Malihabad',
                        pincode: pincode || '226101',
                        cropsGrown: cropsGrown || ['Wheat', 'Paddy', 'Mustard'],
                        farmingExperienceYears: farmingExperienceYears || 8,
                        aadhaarMasked: aadhaarMasked || 'XXXX-XXXX-9876',
                        farmImages: farmImages || ['https://images.unsplash.com/photo-1500382017468-9049fed747ef?auto=format&fit=crop&w=800&q=80'],
                    });
                }
            }
            catch (dbErr) {
                console.warn('[MongoDB Farm Profile Fallback]:', dbErr);
            }
        }
        if (!profile) {
            profile = {
                userId,
                farmName: farmName || `${req.user.name || 'Farmer'}'s Organic Farm`,
                farmSize: farmSize || 5,
                sizeUnit: sizeUnit || 'ACRES',
                soilType: soilType || 'ALLUVIAL',
                soilPh: soilPh || 6.8,
                nitrogen: nitrogen || 140,
                phosphorus: phosphorus || 45,
                potassium: potassium || 40,
                irrigationMethod: irrigationMethod || 'CANAL',
                state: state || 'Uttar Pradesh',
                district: district || 'Lucknow',
                village: village || 'Malihabad',
                pincode: pincode || '226101',
                cropsGrown: cropsGrown || ['Wheat', 'Paddy', 'Mustard'],
                farmingExperienceYears: farmingExperienceYears || 8,
                aadhaarMasked: aadhaarMasked || 'XXXX-XXXX-9876',
                farmImages: farmImages || ['https://images.unsplash.com/photo-1500382017468-9049fed747ef?auto=format&fit=crop&w=800&q=80'],
            };
            inMemoryFarmProfiles.set(userId, profile);
        }
        res.status(200).json({ success: true, message: 'Farm profile saved successfully!', profile });
    }
    catch (error) {
        res.status(500).json({ success: false, message: error.message });
    }
};
