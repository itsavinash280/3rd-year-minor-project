export class PricePredictionEngine {
    static predictCropPrice(cropName, marketLocation = 'Lucknow APMC') {
        const crop = cropName.toLowerCase();
        let basePrice = 2400;
        if (crop.includes('wheat') || crop.includes('gehu'))
            basePrice = 2275;
        else if (crop.includes('rice') || crop.includes('paddy') || crop.includes('dhan'))
            basePrice = 2300;
        else if (crop.includes('mustard') || crop.includes('sarson'))
            basePrice = 5650;
        else if (crop.includes('maize') || crop.includes('makka'))
            basePrice = 2090;
        else if (crop.includes('tomato') || crop.includes('tamatar'))
            basePrice = 1850;
        else if (crop.includes('potato') || crop.includes('aalu'))
            basePrice = 1450;
