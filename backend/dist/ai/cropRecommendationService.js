export class CropRecommendationEngine {
    static recommendCrops(input) {
        ...
        if (normSeason.includes('kharif') || rainfall > 500) {
            // Rice / Paddy
            let riceScore = 85;
            if (['ALLUVIAL', 'CLAY', 'LOAM'].includes(soilType.toUpperCase()))
                riceScore += 8;
            if (rainfall > 800 || irrigationMethod === 'CANAL' || irrigationMethod === 'BOREWELL')
                riceScore += 5;
        ...
        if (normSeason.includes('rabi') || rainfall < 500) {
            // Wheat
            let wheatScore = 90;
            if (['ALLUVIAL', 'BLACK', 'LOAM'].includes(soilType.toUpperCase()))
                wheatScore += 6;
            if (temperature >= 12 && temperature <= 28)
                wheatScore += 3;
            recommendations.push({
